...
export async function mockTrainModel(
  modelId: string,
  parameters: Record<string, any>,
  datasetId: string,
  _trainingConfig: Record<string, any> = {},
...
// Mock status check
export async function mockGetTrainingStatus(
  jobId: string,
...
export async function mockGetAvailableModels(): Promise<ModelListResponse> {
...
// Mock upload dataset
export async function mockUploadDataset(
  _file: File,
  config: {
    name: string;
    description?: string;
