...
export default function BacktestChart() {
  ...
  useEffect(() => {
    ...
    const predictedValues = actualValues.map((val) =>
    ...
    const allPoints = [
      ...
      ...predictedPoints.map((d) => ({
        ...d,
        originalIndex: d.date.getTime(),
      })),
    ];
    ...
    svg
      ...
      .on("mousemove", (event) => {
        ...
        if (d) {
          ...
          const date = d.date;
          const actualPoint = actualPoints.find((p) =>
            p.date.getTime() === date.getTime()
          );
          const predictedPoint = predictedPoints.find((p) =>
            p.date.getTime() === date.getTime()
          );

          hoverLine.attr("x1", xScale(date)).attr("x2", xScale(date)).style(
            "opacity",
            1,
          );
          ...
          const error = Math.abs(actualPoint.value - predictedPoint.value)
            .toFixed(2);
          const errorPercent =
            ((Math.abs(actualPoint.value - predictedPoint.value) /
              actualPoint.value) * 100).toFixed(
                2,
              );
