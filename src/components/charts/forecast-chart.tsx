...
export default function ForecastChart(
  { historicalData, forecastData, confidenceInterval = 95 },
) {
  ...
  useEffect(() => {
    ...
    const yScale = d3
      .scaleLinear()
      .domain([
        d3.min(allData, (d) => (d.lower !== undefined ? d.lower : d.value)) *
        0.9,
        d3.max(allData, (d) => (d.upper !== undefined ? d.upper : d.value)) *
        1.1,
      ])
      .range([height, 0]);
          ...
          if (d.type === "forecast" && d.upper !== undefined) {
            tooltipContent = `
              <div>
                ...
                <br />
                <strong>Upper (${confidenceInterval}%):</strong> ${
              d.upper.toFixed(2)
            }
                <br />
                <strong>Lower (${confidenceInterval}%):</strong> ${
              d.lower.toFixed(2)
            }
              </div>
            `;
    ...
    <div className="w-full h-full">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-4 text-sm">
          ...
          <div className="flex items-center gap-1">
            <div className="w-3 h-0.5 bg-blue-600 border-dashed border-t-2 border-blue-600">
            </div>
            <span>Forecast</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 bg-blue-100 dark:bg-blue-800 rounded-sm">
            </div>
