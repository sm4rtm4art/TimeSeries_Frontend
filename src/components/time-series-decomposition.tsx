...
export default function TimeSeriesDecomposition() {
  ...
  useEffect(() => {
    ...
    const seasonality = Array.from(
      { length: n },
      (_, i) => 15 * Math.sin((i / 7) * Math.PI),
    );
    ...
    // Create axes
    const xAxisOriginal = d3.axisBottom(xScale).ticks(5).tickFormat(
      d3.timeFormat("%b %d"),
    );

    const xAxisTransformed = d3.axisBottom(xScale).ticks(5).tickFormat(
      d3.timeFormat("%b %d"),
    );
