...
export default function ModelComparisonChart({
  ...
  useEffect(() => {
    if (
      !dates || !models || dates.length === 0 || models.length === 0 ||
      dimensions.width === 0
    ) return;
    ...
    // Create scales
    const xScale = d3.scaleTime().domain(d3.extent(parsedDates)).range([
      0,
      width,
    ]);

    const yScale = d3.scaleLinear().domain([minValue, maxValue]).range([
      height,
      0,
    ]);
      ...
      .on("mousemove", (event) => {
        ...
        if (dateIndex >= 0 && dateIndex < parsedDates.length) {
          ...
          filteredModels.forEach((model, i) => {
            ...
            hoverCircles[i].attr("cx", xScale(date)).attr("cy", yScale(value))
              .style("opacity", 1);
          });

          // Update tooltip
          let tooltipContent =
            `<div><strong>Date:</strong> ${date.toLocaleDateString()}<br/>`;
    ...
    filteredModels.forEach((model, i) => {
      ...
      legendItem.append("rect").attr("width", 15).attr("height", 15).attr(
        "rx",
        2,
      ).attr("fill", model.color);
